...
\begin{scope}[font=\footnotesize, text height=1.5ex, text depth=.5ex]
  ...
  \begin{scope}[circuit logic US, every to/.style={out=0,in=180}]
  	...
  	\node[and gate, draw, below right=0 and 0.5 of Opdf] (Opdg) {$g$};
		...
		\draw (Opd.input 1|-Opdf.west) to (Opdf.west);
...
%
	\node[bb={1}{2}, above right=-1 and 6 of X3] (A) {$f$};
...
Operads have been proposed as a way of organizing the various sorts of wiring diagrams \cite{Spivak:2013b,Rupel.Spivak:2013a}. An operad morphism has many inputs and one output, and these correspond to a wiring diagram assembling many components to make one composite. For example, it was shown in \cite{Spivak.Schultz.Rupel:2016a} that the operad for traced monoidal categories is $\Cat{Cob}$, the operad of oriented 1-dimensional cobordisms, and it was shown in \cite{Fong?} that the operad for hypergraph categories is $\Cat{Cospan}$.

While the operad formalism neatly captures wiring composition, and can thus be used to define the above sorts of categorical structures as algebras, there is a technical annoyance that emerges in each case: the operad handles morphisms, but not objects in these categories. The objects must be ``baked in'' to the operad as string labels. Thus it is not the case that traced monoidal categories are algebras on $\Cat{Cob}$; it is the case that for any generating set $\Lambda$ of objects, traced-monoidal-categories-whose-object-set-is-generated-by-$\Lambda$ are algebras on $\Cat{Cob}/\Lambda$. The latter is a mouthful, and is not entirely pleasing.

In this paper we remedy this by following the work of \cite{Joyal.Kock} and \cite{Raynor}. DETAILS.
