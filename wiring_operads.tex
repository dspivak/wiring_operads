...
In this paper we remedy this by following an idea from the work of \cite{Joyal.Kock} that has been further developed by \cite{Raynor}. %\stodo[inline]{Sophie, add details on the history of the monad on presheaves on elementary graphs idea} 
...
\filldraw (3,1) circle (3pt)
	...
	(7,1) circle (3pt);
	 ...
	 \node at (0,-1) {$(\shortmid)$};
%\node at (3,-1) {$|\cc_{\mathbf 0}|$};
%\node at (5,-1) {$|\cc_{\mathbf 1}|$};
%\node at (7,-1) {$|\CX[\two]|$};
% \node at (9,-1) {$|\C_{\mathbf 3}|$};
% \node at (11,-1) {$|\C_{\mathbf 4}|$};

%3

\node at (9,1){\begin{tikzpicture}[scale = 0.5]
\filldraw(0,0) circle(3pt);
\foreach \angle in {-60,60,180} 
{
 \draw[thick] (\angle:0cm) -- (\angle:1cm);
 
     }\end{tikzpicture}};
   %4
   %3

\node at (11,1){\begin{tikzpicture}[scale = 0.5]
\filldraw(0,0) circle(3pt);
\foreach \angle in {-0,90,180,270} 
{
 \draw[thick] (\angle:0cm) -- (\angle:1cm);
     ...
     %\node at (13, -1) {\dots};
\end{tikzpicture}
\caption{ Up to isomorphism, the first few elementary graphs.}\label{elgraph}
}
\end{figure}

 The stick graph is equipped with a non-trivial automorphism $\tau$ satisfying $\tau^2 = \id_{\shortmid}$. %, that changes the orientation of $(\shortmid)$. 
 Further, $\el$ contains all isomorphisms of corollas and for each $n$-corolla $\cc$, there are $2n$ morphisms $(\shortmid) \to \cc$ that can be pictured as inserting $ \shortmid$ into a leg of $\cc$ ``in either $\tau$-direction''. %To each finite set $X$, we may associate a corolla whose legs are labelled by $X$, and it is helpful to think of $\el $ as the groupoid of finite sets and isomorphisms, together with the extra object $ (\shortmid)$ and the morphisms with $(\shortmid)$ as their source.
 
A presheaf $S: \el^{op} \to \set$ of $\el$ then consists of a set $S(\cc)$ for corolla $\cc$, together with the obvious symmetric group actions, and a set of \emph{types} (elsewhere called \emph{colours} or \emph{objects}) $\Lambda = S(\shortmid)$ equipped with an involution. % $ \omega = S(\tau)$. 
...
Crucially for this paper, the object or type-labelling on the edges of a graph, just as the ``morphism-labelling'' on the vertices, is simply part of the data of the presheaf. 
